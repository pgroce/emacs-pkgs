...
- =lang= :: The language of the source block
- =body= :: The body of the source block as a (big) string.
- =properties= :: A plist [fn::I think? It's often accessed in code with =assq= ] containing all the properties of the source block. Keys are =:results=, =:exports=, =:tangle=, =:hlines=, =:noweb=, =:cache=, and =:session=. If a property is not defined, this list contains the default/inherited value.
- =switches= :: The string containing the switches associated with this source block. According to [[https://www.gnu.org/software/emacs/manual/html_node/org/Structure-of-Code-Blocks.html][the documentation]], " \[s]witches provide finer control of the code execution, export, and format". Use of switches is demonstrated in [[https://www.gnu.org/software/emacs/manual/html_node/org/Literal-Examples.html][Literal Examples]]. If no switches are defined, this value is the empty string.
- =name= :: If the block is named (e.g., with =#+NAME:=), this is the name. If no name is supplied, this is(?) the empty string.
- =beg= :: The buffer position beginning this element, as from the =:post-affiliated= property of the [[https://orgmode.org/worg//dev/org-element-api.html][Org Element API]].
- =ref= :: The "\[f]ormat string used to write labels in current block," i.e., the value of the =:label-ref= property if supplied, or the default/inherited value otherwise.
...
It's also helpful sometimes to be able to access an edit buffer's "parent" buffer. This is also not supported directly, but it is possible. The local variables =org-src--beg-marker= and =org-src--end-marker= (both ostensibly private, of course) are [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Markers.html][markers]] referring to the beginning and end of the source block in the parent buffer. One can therefore use =marker-buffer= to get the buffer.
