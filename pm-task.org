...
Right now the only way to work with a task is as text in a buffer, or as an Org element tree. It should be possible to create tasks (and components like comments) a priori. This can, of course, be done by assembling Org element components or using something like =org-ml-from-string=, so this is a low priority.
...
#+begin_src emacs-lisp :noweb-ref src
  ...
  undefined."
    (ts=  (pm-task-status-last-change-timestamp task) timestamp))
...
These functions are trivial extensions of =pm-task-last-change-timestamp=, so they are a low priority to test.
...
Ran 0 tests, 0 results as expected, 0 unexpected (2022-01-09 13:25:38-0500, 0.000063 sec)
#+end_src


***** =pm-task-status-in=                                        :worked:
...
**** Tasks created since \tau                                        :worked:

#+begin_src emacs-lisp :noweb eval :tangle no :results code :wrap src emacs-lisp
<<src>>
  (pg-org-with-src-doc input/basic
    (let* ((el (nth 0 (org-ml-match '(headline) doc)))
           (timestamp (ts-parse-org "[2021-12-11 Sat 20:25]")))
      (pm-task-status-changed-on-or-after timestamp el)))
#+end_src
...
#+begin_src emacs-lisp :noweb yes :wrap src emacs-lisp :results code :tangle no
  ...
  (let ((timestamp (ts-parse-org "[2021-12-11 Sat 20:25]")))
    (pg-org-with-src-doc input/basic
      (->> doc
           (pg-org-match
            '((:and headline
...
**** Tasks resolved since \tau                                       :worked:
...
**** Tasks assigned to \upsilon                                          :worked:
